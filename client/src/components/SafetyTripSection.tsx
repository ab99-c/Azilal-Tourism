...
export default function SafetyTripSection() {
  ...
  return <>
    {showEntryNotice && <div role="dialog" aria-modal="true" aria-labelledby="adrar-safety-notice-title" className="fixed inset-0 z-[100] flex min-h-screen items-center justify-center bg-[#102d23]/80 p-4 backdrop-blur-sm" dir={lang === 'ar' || lang === 'ber' ? 'rtl' : 'ltr'}>
      <div className="w-full max-w-lg rounded-3xl border border-[#d6e4d8] bg-[#f7faf4] p-6 text-center shadow-2xl md:p-9">
        <div className="mx-auto inline-flex items-center gap-2 rounded-full bg-[#dceee4] px-4 py-2 text-sm font-bold text-[#176b4d]"><ShieldCheck className="h-5 w-5" /> ADRAR Safety</div>
        <h1 id="adrar-safety-notice-title" className="mt-5 text-2xl font-black leading-tight text-[#164b38] md:text-3xl">{c.noticeTitle}</h1>
        <p className="mt-4 text-base leading-8 text-[#5b554b]">{c.safety}</p>
        <button type="button" onClick={() => setShowEntryNotice(false)} className="mt-7 w-full rounded-2xl bg-[#176b4d] px-5 py-3.5 font-bold text-white transition hover:bg-[#125b40] focus:outline-none focus:ring-4 focus:ring-[#176b4d]/30">{c.noticeContinue}</button>
      </div>
    </div>}
    <section id="safety-trip" className="py-16 px-4 bg-[#f0f4ec]" dir={lang === 'ar' || lang === 'ber' ? 'rtl' : 'ltr'}>
