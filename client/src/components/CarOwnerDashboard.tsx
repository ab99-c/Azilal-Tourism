...
export default function CarOwnerDashboard() {
  ...
  const { data: dashboardData, refetch } = trpc.dashboard.myCars.useQuery(
    undefined,
    {
      retry: false,
    }
  );
  const { data: myHotels, refetch: refetchHotels } =
    trpc.dashboard.myHotels.useQuery(undefined, { retry: false });
  const { data: myRestaurants, refetch: refetchRestaurants } =
    trpc.dashboard.myRestaurants.useQuery(undefined, { retry: false });
  const { data: myCafes, refetch: refetchCafes } =
    trpc.dashboard.myCafes.useQuery(undefined, { retry: false });
  ...
  >("all");
  const [hotelWhatsAppDrafts, setHotelWhatsAppDrafts] = useState<
  ...
  const createCarMutation = trpc.cars.create.useMutation({
    onSuccess: () => {
      ...
      resetForm();
      toast.success(
        lang === "ar" ? "تمت إضافة السيارة بنجاح" : "Car added successfully"
      );
    },
    onError: err => {
      toast.error(lang === "ar" ? "خطأ في إضافة السيارة" : "Error adding car", {
  ...
  const updateCarMutation = trpc.cars.update.useMutation({
    ...
    onError: err => {
      toast.error(
        lang === "ar" ? "خطأ في تحديث السيارة" : "Error updating car",
        { description: err.message }
      );
    },
  });
  ...
  const updateHotelContactMutation = trpc.hotels.updateContact.useMutation({
    onSuccess: () => {
      refetchHotels();
      toast.success(
        lang === "ar" ? "تم تحديث واتساب الفندق" : "Hotel WhatsApp updated"
      );
    },
    onError: err =>
      toast.error(
        lang === "ar"
          ? "فشل تحديث واتساب الفندق"
          : "Failed to update hotel WhatsApp",
        { description: err.message }
      ),
  });
  ...
  const deleteCarMutation = trpc.cars.delete.useMutation({
    onSuccess: () => {
      ...
      toast.success(
        lang === "ar" ? "تم حذف السيارة بنجاح" : "Car deleted successfully"
      );
    },
    onError: err => {
      toast.error(lang === "ar" ? "خطأ في حذف السيارة" : "Error deleting car", {
        description: err.message,
      });
    },
  });
  ...
  const createRestaurantMutation = trpc.restaurants.create.useMutation({
    ...
    onError: err =>
      toast.error(
        lang === "ar" ? "خطأ في إضافة المطعم" : "Error adding restaurant",
        { description: err.message }
      ),
  });
  const updateRestaurantMutation = trpc.restaurants.update.useMutation({
    ...
    onError: err =>
      toast.error(
        lang === "ar" ? "خطأ في تحديث المطعم" : "Error updating restaurant",
        { description: err.message }
      ),
  });
  const deleteRestaurantMutation = trpc.restaurants.delete.useMutation({
    ...
    onError: err =>
      toast.error(
        lang === "ar" ? "خطأ في حذف المطعم" : "Error deleting restaurant",
        { description: err.message }
      ),
  });
  const createCafeMutation = trpc.cafes.create.useMutation({
    onSuccess: () => {
      ...
      resetForm();
      toast.success(
        lang === "ar" ? "تمت إضافة المقهى بنجاح" : "Café added successfully"
      );
    },
    onError: err =>
      toast.error(lang === "ar" ? "خطأ في إضافة المقهى" : "Error adding café", {
        description: err.message,
      }),
  });
  const updateCafeMutation = trpc.cafes.update.useMutation({
    onSuccess: () => {
      ...
      resetForm();
      toast.success(
        lang === "ar" ? "تم تحديث المقهى بنجاح" : "Café updated successfully"
      );
    },
    onError: err =>
      toast.error(
        lang === "ar" ? "خطأ في تحديث المقهى" : "Error updating café",
        { description: err.message }
      ),
  });
  const deleteCafeMutation = trpc.cafes.delete.useMutation({
    onSuccess: () => {
      ...
      setExpandedId(null);
      toast.success(
        lang === "ar" ? "تم حذف المقهى بنجاح" : "Café deleted successfully"
      );
    },
    onError: err =>
      toast.error(lang === "ar" ? "خطأ في حذف المقهى" : "Error deleting café", {
        description: err.message,
      }),
  });

  // Image upload (restaurants & cafes)
  const uploadRestaurantImageMutation =
    trpc.restaurants.uploadImage.useMutation({
      onSuccess: () => {
        refetchRestaurants();
        toast.success(
  ...
  const removeRestaurantImageMutation =
    trpc.restaurants.removeImage.useMutation({
      onSuccess: () => {
        refetchRestaurants();
        toast.success(
          lang === "ar" ? "تم حذف الصورة بنجاح" : "Image removed successfully"
        );
      },
      onError: err =>
        toast.error(
          lang === "ar" ? "فشل حذف الصورة" : "Failed to remove image",
          { description: err.message }
        ),
    });
  const removeCafeImageMutation = trpc.cafes.removeImage.useMutation({
    ...
    onError: err =>
      toast.error(lang === "ar" ? "فشل حذف الصورة" : "Failed to remove image", {
        description: err.message,
      }),
  });
  const handleRemoveRestaurantImage = (id: number) => {
    if (
      window.confirm(
        lang === "ar" ? "هل أنت متأكد من حذف الصورة؟" : "Remove this photo?"
      )
    ) {
  ...
  const handleImageFile = (file: File) => {
    if (file.size > 4 * 1024 * 1024) {
      toast.error(
        lang === "ar"
          ? "الصورة كبيرة جداً (الحد الأقصى 4 ميغابايت)"
    ...
    reader.onload = () => {
      ...
      if (formKind === "restaurant") {
        if (editingId !== null)
          uploadRestaurantImageMutation.mutate({
            id: editingId,
            base64,
            fileName: file.name,
          });
        ...
        if (editingId !== null)
          uploadCafeImageMutation.mutate({
            id: editingId,
            base64,
            fileName: file.name,
          });
  ...
  const [formKind, setFormKind] = useState<"car" | "restaurant" | "cafe">(
    "car"
  );
  ...
  const handleDeleteCafe = (id: number) => {
    if (
      window.confirm(
        lang === "ar" ? "هل أنت متأكد من حذف هذا المقهى؟" : "Delete this café?"
      )
    )
  ...
  const handleSubmit = () => {
    const base = {
      image: formData.img,
      nameAr: formData.name.ar,
      nameEn: formData.name.en,
      nameFr: formData.name.fr,
      nameBer: formData.name.ber,
      descriptionAr: formData.desc.ar,
      descriptionEn: formData.desc.en,
      descriptionFr: formData.desc.fr,
      descriptionBer: formData.desc.ber,
      locationAr: formData.location.ar,
      locationEn: formData.location.en,
      locationFr: formData.location.fr,
      locationBer: formData.location.ber,
    ...
    if (formKind === "restaurant") {
      const payload = {
        ...base,
        cuisineAr: formData.cuisine.ar,
        cuisineEn: formData.cuisine.en,
        cuisineFr: formData.cuisine.fr,
        cuisineBer: formData.cuisine.ber,
      };
      if (editingId !== null) {
    ...
    if (editingId !== null) {
      updateCarMutation.mutate({
        ...
        nameFr: formData.name.fr,
        nameBer: formData.name.ber,
        typeAr: formData.type,
        typeEn: formData.type,
        typeFr: formData.type,
        typeBer: formData.type,
        descriptionAr: formData.desc.ar,
        descriptionEn: formData.desc.en,
        descriptionFr: formData.desc.fr,
        descriptionBer: formData.desc.ber,
  ...
  const handleReset = () => {
    ...
    toast.info(
      lang === "ar"
        ? "تم إعادة تحميل البيانات الافتراضية"
        : "Default data reloaded"
    );
  };
  ...
  const waitingReviewCount = [
    ...
    ...restaurants,
    ...cafes,
  ].filter((item: any) => item.isActive === false).length;
  ...
  const labels = {
    title: {
      ar: "لوحة تحكم السيارات",
      en: "Car Management",
      fr: "Gestion des Voitures",
      ber: "ⴰⵙⵏⴼⵍ ⵏ ⵜⵙⵍⵍⴰⵙⵜ",
    },
    subtitle: {
      ar: "إضافة وتعديل وحذف السيارات",
      en: "Add, edit and delete vehicles",
      fr: "Ajouter, modifier et supprimer",
      ber: "ⵔⵏⵓ, ⵙⵏⴼⵍ, ⴽⴽⵙ ⵜⵙⵍⵍⴰⵙⵜ",
    },
    addCar: {
      ar: "إضافة سيارة",
      en: "Add Vehicle",
    ...
    whatsappHint: {
      ar: "مثال: 0612345678 أو +212612345678",
      en: "e.g. 0612345678 or +212612345678",
      fr: "ex. 0612345678 ou +212612345678",
      ber: "ⴰⵎⴷⵢⴰ: 0612345678",
    },
    ...
    confirmReset: {
      ar: "هل أنت متأكد؟ سيتم إعادة جميع السيارات للافتراضي",
      en: "Are you sure? All cars will be reset to defaults",
      fr: "Êtes-vous sûr? Toutes les voitures seront réinitialisées",
      ber: "ⵉⵙⵙⵏⴽⴽ ⵜⴳⵉⴷ? ⴰⴷ ⴰⵔⵔⵉⵣⵏⵜ ⵜⵙⵍⵍⴰⵙⵜ",
    },
    yes: { ar: "نعم", en: "Yes", fr: "Oui", ber: "ⵢⵉⵢ" },
    no: { ar: "لا", en: "No", fr: "Non", ber: "ⵓⵀⵓ" },
    empty: {
      ar: "لا توجد سيارات. أضف سيارة جديدة!",
      en: "No vehicles yet. Add your first one!",
      fr: "Aucun véhicule. Ajoutez le premier!",
      ber: "ⵓⵍⵍⵉⵏ ⵜⵙⵍⵍⴰⵙⵜ",
    },
    addNew: {
    ...
    priceHint: {
      ar: "مثال: 400 درهم/يوم",
      en: "e.g. 400 MAD/day",
      fr: "ex: 400 MAD/jour",
      ber: "ⴰⵎⴷⵢⴰ: 400 MAD/ⴰⵙⵙ",
    },
    seatsHint: {
      ar: "مثال: 5 مقاعد",
      en: "e.g. 5 Seats",
    ...
    chooseFile: {
      ar: "اختيار صورة من الجهاز",
      en: "Choose Photo",
      fr: "Choisir une photo",
      ber: "ⵙⵜⵉ ⵜⵡⵍⴰⴼⵜ",
    },
    imgHint: {
      ar: "JPG أو PNG أو WEBP — الحد الأقصى 4 ميغابايت",
      en: "JPG, PNG or WEBP — max 4MB",
      fr: "JPG, PNG ou WEBP — max 4Mo",
      ber: "JPG/PNG/WEBP — ⴰⴼⵓⵙ 4MB",
    },
    uploadPhoto: {
      ar: "رفع صورة",
      en: "Upload Photo",
    ...
    bookingsHint: {
      ar: "الحجوزات الجديدة تظهر هنا — تحقق من الدفع عند وصول العميل وسجله",
      en: "New bookings appear here — verify payment on arrival and record it",
      fr: "Les nouvelles réservations apparaissent ici — vérifiez le paiement à l'arrivée",
      ber: "ⵉⵙⵏⴷⵇⵏ ⵉⵎⴰⵢⵏⵓⵜⵏ ⴷⴰ ⵉⵜⵜⴰⴼⴼⵏ",
    },
    needLogin: {
      ar: "سجل الدخول لرؤية الحجوزات",
      en: "Log in to view bookings",
      fr: "Connectez-vous pour voir les réservations",
      ber: "ⵙⵜⵉⵏ ⴰⵅⵛⵓⵎ",
    },
    tabHotels: { ar: "الفنادق", en: "Hotels", fr: "Hôtels", ber: "ⵉⵙⵏⴷⵇⵏ" },
    emptyHotel: {
      ar: "لا توجد فنادق بعد",
      en: "No hotels yet",
      fr: "Aucun hôtel",
      ber: "ⵓⵍⵍⵉⵏ ⵉⵙⵏⴷⵇⵏ",
    },
    ...
    emptyRestaurant: {
      ar: "لا توجد مطاعم. أضف مطعمك!",
      en: "No restaurants yet. Add yours!",
      fr: "Aucun restaurant. Ajoutez le vôtre!",
      ber: "ⵓⵍⵍⵉⵏ ⵉⵎⵙⵙⴽⵜⵏ",
    },
    emptyCafe: {
      ar: "لا توجد مقاهي. أضف مقهاك!",
      en: "No cafés yet. Add yours!",
      fr: "Aucun café. Ajoutez le vôtre!",
      ber: "ⵓⵍⵍⵉⵏ ⵉⵇⵀⵡⴰⵢⵏ",
    },
    cuisine: {
      ar: "نوع المأكولات",
    ...
    confirmDeleteRestaurant: {
      ar: "هل أنت متأكد؟ سيتم حذف هذا المطعم",
      en: "Are you sure? This restaurant will be deleted",
      fr: "Êtes-vous sûr? Ce restaurant sera supprimé",
      ber: "ⵉⵙⵙⵏⴽⴽ ⵜⴳⵉⴷ? ⴰⴷ ⵉⵜⵜⵡⴰⴽⴽⵙ ⵉⵎⵙⵙⴽ",
    },
    confirmDeleteCafe: {
      ar: "هل أنت متأكد؟ سيتم حذف هذا المقهى",
      en: "Are you sure? This café will be deleted",
      fr: "Êtes-vous sûr? Ce café sera supprimé",
      ber: "ⵉⵙⵙⵏⴽⴽ ⵜⴳⵉⴷ? ⴰⴷ ⵉⵜⵜⵡⴰⴽⴽⵙ ⴰⵇⵀⵡⴰ",
    },
    bookingTypeRestaurant: {
      ar: "مطعم",
      en: "Restaurant",
      fr: "Restaurant",
      ber: "ⵉⵎⵙⵙⴽ",
  ...
  const l = (key: keyof typeof labels) =>
    labels[key][lang as Lang] || labels[key].en;
  ...
  const getHotelWhatsAppDraft = (hotel: any) =>
    hotelWhatsAppDrafts[hotel.id] ?? hotel.whatsapp ?? "";
  ...
  return (
    <section
      id="owner-dashboard"
      className="py-16 bg-gradient-to-b from-[#1b5e3f] to-[#0f3d28]"
    >
      <div className="container" dir={dir}>
        ...
        <div className="text-center mb-10">
          ...
          {user?.role === "admin" && (
            <div
              role="status"
              className={`mx-auto mt-5 max-w-md rounded-xl border px-4 py-3 text-center text-sm ${dbHealthQuery.isLoading ? "border-white/20 bg-white/10 text-white/70" : dbHealthQuery.data?.ok ? "border-emerald-300/30 bg-emerald-500/10 text-emerald-100" : "border-amber-300/40 bg-amber-500/15 text-amber-100"}`}
            >
              {dbHealthQuery.isLoading
                ? "كنفحصو الاتصال بقاعدة البيانات..."
                : dbHealthQuery.data?.ok
                  ? `قاعدة البيانات خدامة · ${dbHealthQuery.data.latencyMs}ms`
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 mt-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl px-6 py-3 flex items-center gap-3 border border-white/10">
              ...
              <span className="text-2xl font-bold text-[#c8a951]">
                {cars.length}
              </span>
            </div>
            {bookingsData && (
        ...
        <div className="mb-8 overflow-x-auto pb-1">
          <div className="mx-auto flex w-max min-w-max bg-white/10 rounded-xl p-1 backdrop-blur-sm border border-white/10">
            ...
            >
              <Building2 className="w-4 h-4 inline me-1.5" />
              {l("tabHotels")}
            </button>
            <button
            ...
            >
              <Utensils className="w-4 h-4 inline me-1.5" />
              {l("tabRestaurants")}
            </button>
            <button
            ...
            >
              <Coffee className="w-4 h-4 inline me-1.5" />
              {l("tabCafes")}
            </button>
            <button
            ...
            >
              <ClipboardList className="w-4 h-4 inline me-1.5" />
              {l("tabBookings")}
            </button>
            <button
            ...
            >
              <Clock className="w-4 h-4 inline me-1.5" />
              {l("tabAvailability")}
            </button>
            {isAdmin && (
              <button
            ...
            {isAdmin && (
              ...
              >
                <ShieldCheck className="w-4 h-4 inline me-1.5" />
                {lang === "ar"
                  ? "المراجعة"
                  : lang === "fr"
                    ? "Révision"
        ...
        {!isAdmin && waitingReviewCount > 0 && (
          <div className="mx-auto mb-6 max-w-3xl rounded-xl border border-[#c8a951]/35 bg-[#c8a951]/10 px-4 py-3 text-center text-sm text-white/90">
            {lang === "ar"
              ? `عندك ${waitingReviewCount} إعلان كيتسنى المراجعة قبل ما يبان للزوار.`
              : lang === "fr"
                ? `${waitingReviewCount} annonce(s) attend(ent) une révision avant publication.`
                : lang === "ber"
                  ? `${waitingReviewCount} ⵜⵉⵔⵔⴰ ⵜⵜⵔⴰⵊⵓⵏⵜ ⴰⵙⵙⵉⵇⵙⵉ ⵣⵣⴰⵜ ⴰⴷ ⵜⵜⵓⵙⴽⴰⵏⵏⵜ.`
                  : `${waitingReviewCount} listing(s) await review before visitors can see them.`}
          </div>
        )}
        ...
        {activeTab === "restaurants" && (
          <div className="flex flex-wrap gap-3 mb-8 justify-center">
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={() => openAddForm("restaurant")}
              className="px-5 py-2.5 bg-[#c8a951] text-[#1b5e3f] rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-[#d4b75e] transition-colors"
            >
              <Plus className="w-4 h-4" />
              {l("addRestaurant")}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-4">
                  {[
                    ...
                    [
                      ...
                      bookingMetrics?.pending ?? 0,
                    ],
                    [
                      lang === "ar" ? "مؤكد" : "Confirmed",
                      bookingMetrics?.confirmed ?? 0,
                    ],
                    [
                      lang === "ar" ? "مكتمل" : "Completed",
                      bookingMetrics?.completed ?? 0,
                    ],
                    [
                      lang === "ar" ? "ملغى" : "Cancelled",
                      bookingMetrics?.cancelled ?? 0,
                    ],
                  ].map(([label, value]) => (
                    <div
                      key={String(label)}
                    ...
                    >
                      <p className="text-lg font-black text-[#c8a951]">
                        {value}
                      </p>
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-start">
                        <div>
                          ...
                          <span
                            className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${b.type === "hotel" ? "bg-[#1b5e3f] text-[#c8a951]" : "bg-[#c8a951] text-[#1b5e3f]"}`}
                          >
                            {b.type === "hotel"
                              ? l("bookingTypeHotel")
                              : b.type === "restaurant"
                                ? l("bookingTypeRestaurant")
                                : b.type === "cafe"
                                  ? l("bookingTypeCafe")
                                  : l("bookingTypeCar")}
                          </span>
                        ...
                        <div>
                          ...
                          <p
                            className={`text-xs font-bold flex items-center gap-1 mt-0.5 ${isPaid ? "text-[#c8a951]" : "text-white/70"}`}
                          >
                            {isPaid ? (
                          ...
                          {b.totalPrice && (
                            <p className="text-[#c8a951] text-xs font-semibold mt-1">
                              {b.totalPrice}
                            </p>
                          )}
                        </div>
                        <div className="flex flex-col gap-2">
                          {!isPaid && (
                            <button
                              onClick={() =>
                                markPaidMutation.mutate({ id: b.id })
                              }
                          ...
                          {!isConfirmed && (
                            <button
                              onClick={() =>
                                confirmMutation.mutate({ id: b.id })
                              }
                          ...
                          {isConfirmed && (
                            <>
                              ...
                              <button
                                onClick={() =>
                                  completeMutation.mutate({ id: b.id })
                                }
                                disabled={completeMutation.isPending}
                                className="px-3 py-1.5 bg-[#1b5e3f] text-[#c8a951] rounded-lg text-xs font-bold flex items-center justify-center gap-1 hover:bg-[#247a51] transition-colors border border-[#c8a951]/30 disabled:opacity-50"
                              >
                                <CheckCircle2 className="w-3.5 h-3.5" />
                                {lang === "ar"
                                  ? "تم الإنجاز"
                          ...
                          <WhatsAppButton
                            phone={b.guestPhone}
                            message={getCustomerWhatsAppMessage(
                              lang,
                              b.itemName
                            )}
                            label={
                  ...
                  >
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-16 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-white/10">
                        {hotel.image ? (
                          ...
                          <Building2 className="w-full h-full p-3 text-white/30" />
                        )}
                      </div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={e =>
                          setHotelWhatsAppDraft(hotel.id, e.target.value)
                        }
                      ...
                      <button
                        onClick={() =>
                          updateHotelContactMutation.mutate({
                            id: hotel.id,
                            whatsapp: getHotelWhatsAppDraft(hotel),
                          })
                        }
              ...
              {restaurants.map((item: any, i: number) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="text-[#c8a951] font-bold text-sm">
                      ★ {Number(item.rating) || item.rating}
                    </div>
                    <motion.div
                        ...
                        <div className="px-4 pb-4 pt-2 border-t border-white/10">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                uploadExistingRestaurantImage(item);
                              }}
                              disabled={uploadRestaurantImageMutation.isPending}
                              className="flex-1 py-2 bg-white/10 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-white/20 transition-colors disabled:opacity-50"
                            >
                            ...
                            {item.image && (
                              <button
                                onClick={e => {
                                  e.stopPropagation();
                                  handleRemoveRestaurantImage(item.id);
                                }}
                                disabled={
                                  removeRestaurantImageMutation.isPending
                                }
                                className="flex-1 py-2 bg-orange-500/20 text-orange-400 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-orange-500/30 transition-colors disabled:opacity-50"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                                {l("removePhoto")}
                              </button>
                        ...
                        <div className="px-4 pb-4 pt-2 border-t border-white/10">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                uploadExistingCafeImage(item);
                              }}
                              disabled={uploadCafeImageMutation.isPending}
                              className="flex-1 py-2 bg-white/10 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-white/20 transition-colors disabled:opacity-50"
                            >
                              <Upload className="w-3.5 h-3.5" />
                              {l("uploadPhoto")}
                            ...
                            {item.image && (
                              <button
                                onClick={e => {
                                  e.stopPropagation();
                                  handleRemoveCafeImage(item.id);
                                }}
                                disabled={removeCafeImageMutation.isPending}
                                className="flex-1 py-2 bg-orange-500/20 text-orange-400 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 hover:bg-orange-500/30 transition-colors disabled:opacity-50"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                                {l("removePhoto")}
              ...
              >
                ...
                <div className="bg-[#1b5e3f] rounded-t-3xl px-6 py-4 flex items-center justify-between">
                  <h3 className="text-white font-bold text-lg">
                    {editingId !== null ? l("editCar") : l("addNew")}
                  </h3>
                  <button
                    onClick={resetForm}
                ...
                <div className="p-6 space-y-4">
                  ...
                  {formKind !== "car" && (
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          lang === "ar"
                            ? "مثال: وسط مدينة أزيلال"
                            : "e.g. Azilal City Center"
                        }
                        className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#1b5e3f]/20 focus:border-[#1b5e3f] outline-none text-sm"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={l("whatsappHint")}
                      className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-[#25D366]/30 focus:border-[#25D366] outline-none text-sm"
                    />
                ...
                <div className="px-6 py-4 bg-gray-50 rounded-b-3xl flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 bg-[#1b5e3f] text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-[#0f3d28] transition-colors disabled:opacity-50"
                  >
                    <Save className="w-4 h-4" />
                    {l("save")}
                  </button>
