...
export default function BookingModal({
  ...
  const t_book = (key: string) => {
    const translations: Record<Lang, Record<string, string>> = {
      ar: {
        ...
        "booking.payOnArrivalDesc":
          "أنت تدفع مباشرة للفندق أو صاحب السيارة عند الوصول — بدون أي رسوم إضافية",
        "booking.payOnArrivalNote":
          "⚠️ الدفع عند الوصول / الدفع نقدًا أو بتحويل بنكي",
        ...
        "booking.submitting": "جارٍ إرسال طلب الحجز...",
        "booking.datesInvalid":
          "يجب أن يكون تاريخ المغادرة أو الإرجاع بعد تاريخ الوصول أو الاستلام.",
      ...
      en: {
        ...
        "booking.payOnArrivalDesc":
          "You pay directly to the hotel or car owner upon arrival — no extra fees",
        "booking.payOnArrivalNote":
          "⚠️ Pay on arrival / pay in cash or by bank transfer",
        "booking.total": "Estimated Total",
        "booking.nights": "night(s)",
        "booking.days": "day(s)",
        "booking.confirmPay": "Confirm Booking — Pay on Arrival",
        "booking.submitting": "Sending your booking request...",
        "booking.datesInvalid": "The end date must be after the start date.",
        "booking.checkingAvailability":
      ...
      fr: {
        ...
        "booking.payOnArrivalDesc":
          "Vous payez directement à l'hôtel ou au propriétaire de la voiture à votre arrivée — sans frais supplémentaires",
        "booking.payOnArrivalNote":
          "⚠️ Paiement sur place / en espèces ou par virement",
        "booking.total": "Total estimé",
        "booking.nights": "nuit(s)",
        "booking.days": "jour(s)",
        "booking.confirmPay": "Confirmer — Paiement sur place",
        "booking.submitting": "Envoi de votre demande de réservation…",
        "booking.datesInvalid":
      ...
      ber: {
        ...
        "booking.payOnArrivalDesc":
          "ⵜⵅⵍⴰⵙⴷ ⵉ ⵜⴰⵔⵉⴽⵜ ⵏⵖ ⴱⴰⴱ ⵏ ⵜⴰⵙⵍⵍⴰⵙⵜ ⵃⵜⵜⴰ ⴷ ⴰⵔⵔⴰⵡ",
        ...
        "booking.submitting": "ⴰⵔ ⵏⴰⵣⵏ ⴰⵙⵓⵜⵔ ⵏ ⵓⵃ⟟⟟ⵓ...",
  ...
  const handleSubmit = () => {
    if (
      ...
      toast.error(
        availabilityQuery.data?.available === false
          ? t_book("booking.datesUnavailable")
          : t_book("booking.datesInvalid")
      );
      return;
    }
          ...
          >
            ...
            <div className="relative bg-gradient-to-br from-[#1b5e3f] to-[#0f3d28] p-6 pb-8">
              ...
              <h3 className="text-xl font-bold text-white mb-1">
                {t_book(
                  type === "hotel" ? "booking.title.hotel" : "booking.title.car"
                )}
              </h3>
              ...
              {step === 1 && (
                ...
                >
                  ...
                  {(errors.dateRange ||
                    ...
                    (hasValidDateRange && availabilityQuery.data)) && (
                    <div
                      role="status"
                    ...
                    >
                      {errors.dateRange
                        ...
                        : errors.availability ||
                          ...
                          ? t_book("booking.datesUnavailable")
                          : availabilityQuery.isFetching
                            ? t_book("booking.checkingAvailability")
                            : availabilityQuery.isError
                              ? t_book("booking.availabilityError")
                              : t_book("booking.datesAvailable")}
                    </div>
                  )}
                  ...
                  <div className="bg-[#f5f5f0] rounded-xl p-4 flex items-center justify-between">
                    <span className="text-sm text-gray-600">
                      {t_book("booking.price")}
                    </span>
                    <span className="text-lg font-bold text-[#1b5e3f]">
                      {price}{" "}
                      <span className="text-xs font-normal text-gray-400">
                        {type === "hotel"
                          ? t_book("booking.perNight")
                          : t_book("booking.perDay")}
                      </span>
                    </span>
                  </div>
                </motion.div>
                    ...
                    <div className="border-2 border-[#1b5e3f] bg-[#1b5e3f]/5 rounded-xl p-4 flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-bold text-gray-800 text-sm">
                          {t_book("booking.payOnArrival")}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">
                          {t_book("booking.payOnArrivalDesc")}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#f5f5f0] rounded-xl p-4">
                    ...
                    <div className="border-t border-gray-200 pt-2 flex items-center justify-between">
                      <span className="text-sm font-semibold text-gray-700">
                        {t_book("booking.total")}
                      </span>
                      <span className="text-xl font-bold text-[#1b5e3f]">
                        {computedTotal !== null
                          ? `${Math.round(computedTotal).toLocaleString()} MAD`
                          : price}
                      </span>
                    </div>
                  </div>
                ...
                >
                  {isSubmitting ? (
                    <motion.div
                      role="status"
                      aria-live="polite"
                      aria-label={t_book("booking.submitting")}
                      initial={{ opacity: 0, y: 8 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="text-center py-8"
                    >
                      <div className="relative mx-auto mb-4 flex h-14 w-14 items-center justify-center">
                        <div className="absolute inset-0 rounded-full border-4 border-[#1b5e3f]/15" />
                        <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-[#1b5e3f] animate-spin" />
                        <Loader2
                          className="h-5 w-5 animate-spin text-[#1b5e3f]"
                          aria-hidden="true"
                        />
                      </div>
                      <p className="font-semibold text-[#1b5e3f]">
                        {t_book("booking.submitting")}
                      </p>
                      <div
                        className="mx-auto mt-3 flex w-24 items-center justify-center gap-1"
                        aria-hidden="true"
                      >
                        {[0, 1, 2].map(dot => (
                          <motion.span
                            key={dot}
                            className="h-1.5 w-1.5 rounded-full bg-[#1b5e3f]"
                            animate={{
                              opacity: [0.25, 1, 0.25],
                              scale: [0.85, 1, 0.85],
                            }}
