...
describe("connection status indicator", () => {
  it("supports online, weak, and offline states in all supported languages", () => {
    ...
    expect(source).toContain('ar: { label: "الاتصال مزيان"');
  ...
  it("is accessible, responsive, and mounted on both app flows", () => {
    expect(source).toContain('role="status"');
    expect(source).toContain('aria-live="polite"');
    expect(source).toContain("max-w-[calc(100vw-1.5rem)]");
    expect(source).toContain("motion-reduce:animate-none");
    expect(app).toContain("<ConnectionStatusIndicator />");
    expect(app.match(/<ConnectionStatusIndicator \/>/g)?.length).toBe(2);
  });
});
